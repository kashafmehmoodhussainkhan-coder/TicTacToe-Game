...
private:
    ...
    char symbol;
public:
    player(string p, char s, board& ref): name(p), symbol(s), bobj(ref) {}
void makemove() {
     int row, col;
      while(true) {
      ...
      break;}
      else { cout << "Cell already occupied. Try again.\n";    }}
      else {
...
private:
    ...
    bool player1turn;
public:
    game(player p1, player p2, board& Board): player1(p1), player2(p2), b(Board), player1turn(true) {}
void run() {
    while(true) {
    b.display();
    if(player1turn)
    ...
    if(checkwin()) {
    b.display();
    cout << (player1turn ? player1.getName() : player2.getName())<< " WINS!\n";
    break;
    }
    if(b.isFull()) {
    b.display();
